...
* Last Modified : Tue Aug 25 19:17:31 2020
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < vstr_rgb_filepath.size(); ++i) {
    ...
    vo.Track(target_color, target_depth, std::stod(vstr_timestamp[i]));
  }
  auto poses = vo.FusionTwoTrajectory();
  for (int i = 0; i < vstr_rgb_filepath.size(); ++i) {
    //change se3 to tum pose format(timestamp, tx, ty, tx, qx, qy, qz, qw) and output
    Eigen::Isometry3d current_pose_iso;
    current_pose_iso= poses[i];
