...
class ElasticFusion
{
    ...
    public:
        EFUSION_API ElasticFusion(const int timeDelta = 200,
                      ...
                      const bool _useICPORB_VO = true,
                      const bool _useICPDepthAlone = false,
         ...
         * @param val
         */
        EFUSION_API void setEASICPORB(const bool & val);
         ...
         * Calculate the above for the current frame (only done on the first frame normally)
         */
        EFUSION_API void setICPORB(bool icporb, bool icpalone){
	   useICPORB_VO = icporb;
	   useICPDepthAlone = icpalone;
	}
        //Here be dragons
    private:
        IndexMap indexMap;
