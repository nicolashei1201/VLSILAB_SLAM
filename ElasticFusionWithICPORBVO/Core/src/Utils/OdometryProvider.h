...
class OdometryProvider
{
    public:
        ...
        static inline void computeUpdateSE3Fake(Eigen::Matrix<double, 4, 4, Eigen::RowMajor> & resultRt, const Eigen::Matrix<double, 6, 1> & result, Eigen::Isometry3f & rgbOdom)
        {
            // for infinitesimal transformation
            Eigen::Matrix<double, 4, 4, Eigen::RowMajor> Rt = Eigen::Matrix<double, 4, 4, Eigen::RowMajor>::Identity();
            ...
            Rt(1, 3) = result(1);
            Rt(2, 3) = result(2);
            Eigen::Matrix<double, 4, 4, Eigen::RowMajor> resultRtcp;
            resultRtcp = Rt * resultRt;

            Eigen::Matrix<double, 3, 3, Eigen::RowMajor> rotation = resultRtcp.topLeftCorner(3, 3);
            rgbOdom.setIdentity();
            rgbOdom.rotate(rotation.cast<float>().eval());
            rgbOdom.translation() = resultRtcp.cast<float>().eval().topRightCorner(3, 1);
        }
};
