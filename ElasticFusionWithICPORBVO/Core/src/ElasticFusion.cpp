...
ElasticFusion::ElasticFusion(const int timeDelta,
                             ...
                             const bool _useICPORB_VO,
                             const bool _useICPDepthAlone,
 ...
 : frameToModel(Resolution::getInstance().width(),
   ...
   fernDeforms(0),
   consSample(20),
   useICPORB_VO(_useICPORB_VO),
   useICPDepthAlone(_useICPDepthAlone),
                                 ...
                                 const bool bootstrap)
{
    ...
    else
    {
      ...
      //Eigen::Matrix4f lastPose = poseGraph.back().second;
      //std::cout<<"last pose:\n"<< lastPose<<"\n";
      ...
      TOCK("autoFill");
			// fillInForICP.vertex(indexMap.vertexTex(), textures[GPUTexture::DEPTH_FILTERED], 1);
      if (useICPORB_VO) {
        ...
        if (useICPDepthAlone) {
          ...
          Eigen::Matrix4f ICP_relative = icporb_vo->GetRelativePose(1).matrix().cast<float>();
          //currPose = currPose * icporb_vo->GetRelativePose(1).matrix().cast<float>();
          currPose = currPose * ICP_relative;
          //std::cout<<"ICP Relative:\n"<<ICP_relative<<"\n";
        ...
        {
            currPose = *inPose;
            Eigen::Matrix4f inPose_Value = *inPose;
            std::cout<<"eat pose!!\n";
            std::cout<<"inPose : \n"<< inPose_Value<<"\n";
            std::cout<<"last pose:\n"<< poseGraph.back().second<<"\n";;
            //currPose =  lastPose * inPose_Value;
            currPose = *inPose;
        }
      } // end of else ORBICP
        ...
        if(!lost && closeLoops && rawGraph.size() == 0)
        {
            ...
            std::cout<<"lastICPCount :"<<modelToModel.lastICPCount<<"\n";
            std::cout<<"lastICPError :"<<modelToModel.lastICPError<<"\n";
...
void ElasticFusion::savePly()
{
    ...
    //get deformation node in ply
    const std::vector<GraphNode*> & graph = getLocalDeformation().getGraph();
    std::ofstream fs2;
    fs2.open (filename.append("_node.ply"));

    fs2 << "ply";
    fs2 << "\nformat " << "ascii" << " 1.0";

    // Vertices
    fs2 << "\nelement vertex "<< graph.size();
    fs2 << "\nproperty float x"
          "\nproperty float y"
          "\nproperty float z";
    fs2 << "\nend_header\n";
    ...
    {
        fs2 << graph.at(i)->position(0);
        fs2 <<" ";
        fs2 << graph.at(i)->position(1);
        fs2 <<" ";
        fs2 << graph.at(i)->position(2);
        fs2 << "\n";
    }
    fs2.close();
    delete [] mapData;
...
void ElasticFusion::setEASICPORB(const bool & val)
{
    useICPORB_VO = val;
}
