   ...
   resizeStream(0)
{
    ...
    cv::FileStorage fs ("../../../ICPORB_VO/settings/TUM/TUM1Elastic.yaml", cv::FileStorage::READ);
    ...
    openLoop = Parse::get().arg(argc, argv, "-o", empty) > -1;
    ...
    useICPORB = Parse::get().arg(argc, argv, "-ICP", empty) > -1;
    useICPORBDA = Parse::get().arg(argc, argv, "-ICPDA", empty) > -1;
    rewind = Parse::get().arg(argc, argv, "-r", empty) > -1;
    frameToFrameRGB = Parse::get().arg(argc, argv, "-ftf", empty) > -1;

    doOptimise = Parse::get().arg(argc, argv, "-Opt", empty) > -1;
    ...
    gui->useEASICPORB->Ref().Set(useICPORB);
    gui->useEASICPORBDA->Ref().Set(useICPORBDA);
...
void MainController::loadCalibration(const std::string & filename)
{
    ...
    int n = sscanf(line.c_str(), "%lg %lg %lg %lg %lg %lg", &fx, &fy, &cx, &cy, &width, &height);

    assert(n == 6 && "Ooops, your calibration file should contain a single line with fx fy cx cy!");
...
void MainController::run()
{
    while(!pangolin::ShouldQuit() && !((!logReader->hasMore()) && quiet) && !(eFusion->getTick() == end && quiet))
    {
        ...
        if(gui->drawDefGraph->Get())
        {
            ...
            {
                pangolin::glDrawCross(graph.at(i)->position(0), // x coordinate
                                      graph.at(i)->position(1), // y coordinate
                                      graph.at(i)->position(2), // z coordinate
                                      0.1);
        ...
        eFusion->setEASICPORB(gui->useEASICPORB->Get());
